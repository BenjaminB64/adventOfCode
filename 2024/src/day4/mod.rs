use aoc_runner_derive::{aoc};

pub fn parse_input_day4_part1(input: &str) -> u32 {
    // for each char, if it's m, try to catch mul(int,int)
    let mut i = 0;
    let look_for = "XMAS";
    let look_for_reverse = "SAMX";
    let look_for_bytes = look_for.as_bytes();
    let look_for_reverse_bytes = look_for_reverse.as_bytes();

    let look_for_len = look_for.len() as u8;
    let mut line_len: u16 = 0;
    let mut nb_found = 0;
    let input_as_bytes = input.as_bytes();
    let input_len: u32 = input.len() as u32;

    while i < input_len {
        if input_as_bytes[i as usize] == '\n' as u8 {
            line_len = i as u16;
            break;
        }
        i += 1;
    }
    let nb_lines = input_len / line_len as u32;
    let nb_lines_limit = nb_lines - 3;
    let k_limit = line_len - 3;

    // show all lines
    for i in 0..nb_lines {
        for k in 0..line_len {
            let c = get_char_x_y_from_string(input_as_bytes, k, i as u16, line_len);
            if c != look_for_bytes[0] && c != look_for_reverse_bytes[0] {
                continue;
            }
            if k < k_limit {
                let right = get_string_right(input_as_bytes, k, i as u16, &line_len);
                if check_string(&right, look_for_bytes, look_for_reverse_bytes) {
                    // println!("Found right at x: {}, y: {}", k, i);
                    nb_found += 1;
                }
            }

            if i < nb_lines_limit {
                let bottom = get_string_bottom(input_as_bytes, k, i as u16, &line_len);
                if check_string(&bottom, look_for_bytes, look_for_reverse_bytes) {
                    // println!("Found bottom at x: {}, y: {}", k, i);
                    nb_found += 1;
                }
                if k < k_limit {
                    let right_bottom = get_string_right_bottom(input_as_bytes, k, i as u16, &line_len);
                    if check_string(&right_bottom, look_for_bytes, look_for_reverse_bytes) {
                        // println!("Found right bottom at x: {}, y: {}", k, i);
                        nb_found += 1;
                    }
                }
            }

            if k >= 3 && i < nb_lines_limit {
                let left_bottom = get_string_left_bottom(input_as_bytes, k, i as u16, &line_len);
                if check_string(&left_bottom, look_for_bytes, look_for_reverse_bytes) {
                    // println!("Found left bottom at x: {}, y: {}", k, i);
                    nb_found += 1;
                }
            }
        }
    }
    nb_found
}

pub fn parse_input_day4_part2(input: &str) -> u32 {
    // for each char, if it's m, try to catch mul(int,int)
    let mut i = 0;
    let look_for = "MAS";
    let look_for_reverse = "SAM";
    let look_for_bytes = look_for.as_bytes();
    let look_for_reverse_bytes = look_for_reverse.as_bytes();

    let look_for_len = look_for.len() as u8;
    let mut line_len: u16 = 0;
    let mut nb_found = 0;
    let input_as_bytes = input.as_bytes();
    let input_len: u32 = input.len() as u32;

    while i < input_len {
        if input_as_bytes[i as usize] == '\n' as u8 {
            line_len = i as u16;
            break;
        }
        i += 1;
    }
    let nb_lines = input_len / line_len as u32;
    let nb_lines_limit = nb_lines - 1;
    let k_limit = line_len - 1;

    // println!("line_len: {}, nb_lines: {}, nb_lines_limit: {}, k_limit: {}", line_len, nb_lines, nb_lines_limit, k_limit);
    // show all lines
    for i in 0..nb_lines {
        for k in 0..line_len {
            let c = get_char_x_y_from_string(input_as_bytes, k, i as u16, line_len);
            if c != 'A' as u8 {
                continue;
            }
            if i < nb_lines_limit && k < k_limit && i >= 1 && k >= 1 {
                // println!("k: {}, i: {}, c: {}", k, i, c as char);
                let right_bottom = get_string_right_bottom(input_as_bytes, (k - 1) as u16, (i - 1) as u16, &line_len);
                if check_string(&right_bottom, look_for_bytes, look_for_reverse_bytes) {
                    // println!("Found right bottom at x: {}, y: {}", k, i);
                    let left_bottom = get_string_left_bottom(input_as_bytes, (k + 1) as u16, (i - 1) as u16, &line_len);
                    if check_string(&left_bottom, look_for_bytes, look_for_reverse_bytes) {
                        // println!("Found left bottom at x: {}, y: {}", k, i);
                        nb_found += 1;
                    }
                }
            }
        }
    }
    nb_found
}

pub fn check_string(s: &[u8], look_for: &[u8], look_for_reverse: &[u8]) -> bool {
    s == look_for || s == look_for_reverse
}

pub fn get_string_right(s: &[u8], x: u16, y: u16, line_len: &u16, len: u16) -> Box<[u8]> {
    let start = (y * (line_len + 1) + x) as usize;
    s[start..(start + len as usize)].to_vec().into_boxed_slice()
}

pub fn get_string_bottom<T>(s: &[u8], x: u16, y: u16, line_len: &u16) -> T {
    let mut ret: T = ();
    for i in 0..3u16 {
        ret[i as usize] = get_char_x_y_from_string(s, x, y + i, *line_len);
    }
    ret
}

pub fn get_string_left_bottom<T>(s: &[u8], x: u16, y: u16, line_len: &u16) -> T {
    let mut ret: T = ();
    for i in 0..ret.len() {
        ret[i as usize] = get_char_x_y_from_string(s, x - i, y + i, *line_len);
    }
    ret
}

pub fn get_string_right_bottom<T>(s: &[u8], x: u16, y: u16, line_len: &u16) -> T {
    let mut ret: T = ();
    for i in 0..ret.len() {
        ret[i as usize] = get_char_x_y_from_string(s, x + i, y + i, *line_len);
    }
    ret
}

pub fn get_char_x_y_from_string(s: &[u8], x: u16, y: u16, line_len: u16) -> u8 {
    s[x_y_to_index(x, y, line_len)]
}

pub fn x_y_to_index(x: u16, y: u16, line_len: u16) -> usize {
    (y * (line_len + 1) + x) as usize
}

#[aoc(day4, part1)]
pub fn part1(input: &str) -> u32 {
    //WIPparse_input_day4_part1(input)
    0
}

#[aoc(day4, part2)]
pub fn part2(input: &str) -> u32 {
    parse_input_day4_part2(input)
}

